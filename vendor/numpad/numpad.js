var numpad = {
  ...
  init: function(){
    ...
    // Backspace
    buttonator("<i class=\"fas fa-backspace\"></i>", "del", numpad.delete);
    // 4 to 6
    for (let i=4; i<=6; i++) { buttonator(i, "num", numpad.digit); }
    // Clear
    buttonator("<i class=\"fas fa-undo\"></i>", "clr", numpad.reset);
    // 1 to 3
    for (let i=1; i<=3; i++) { buttonator(i, "num", numpad.digit); }
    // Cancel
    buttonator("<i class=\"fas fa-times\"></i>", "cx", numpad.hide);
    // 0
    buttonator(0, "zero", numpad.digit);
    // .
    buttonator(".", "dot", numpad.dot);
    // OK
    buttonator("<i class=\"fas fa-check\"></i>", "ok", numpad.select);
